...
const formatTitle = (dataString) => {
  const len = dataString.length
  return (dataString.substr(0,len - 3).split('＆').join(' | '))
}

const formatContent = (dataString) => {
  const splitter = (dataString.indexOf('from')>0)?'from':'by'
  return dataString.split(splitter)
}
